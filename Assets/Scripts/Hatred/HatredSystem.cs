...
public class HatredSystem : MonoBehaviour
{
    ...
    public int hateValueDecrement = 2;         //每秒仇恨值减少量
    ...
    {
        Dictionary<Individual, int>.KeyCollection HLkeys = hatredMap.Keys;
        if (HLkeys.Count == 0)
        {
            StartCoroutine(HateDecrementTimer());
            Debug.Log("No Hate!!!");
        ...
        Individual[] keyArray = new Individual[HLkeys.Count];

        int index = 0;
        foreach (Individual key in HLkeys)
        {
            keyArray[index] = key;
            index++;
        }
        ...
        for (int i = 0; i < HLkeys.Count; i++)
        {
            ...
            Debug.Log("减少了对" + keyArray[i].gameObject.name + "的" + hateValueDecrement + "点仇恨值，当前仇恨值是：" + hatredMap[keyArray[i]]);

            //仇恨值小于0移除出仇恨表
            if (hatredMap[keyArray[i]] <= 0)
            {
                Debug.Log("Like!!!");
